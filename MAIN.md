...
| `PATCH` で現在値を確認せずに更新する | 教材の課題では単純化しているが、実務では同じデータを複数人が同時に編集する可能性がある。後から来た更新が先に保存された変更を上書きしないように、`updated_at`、version 番号、ETag と `If-Match` などを使った楽観ロックや、必要に応じた排他制御を検討する。 |
