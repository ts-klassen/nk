...
この教材では、完成済みの `system-a` を見ながら、同じ作りの `system-b` を実装する。

`system-a` を最初から最後まで読んでから `system-b` を作るのではない。機能ごとに、見本を読む、動かす、課題側に実装する、確認する、を繰り返す。

第1章から順に進める。第5章までは、バックエンドの基本と、作業に必要な準備を扱う。第6章から `system-b` の実装に入る。

各章では、その章で出てくる言葉と実装方法を説明してから作業する。
