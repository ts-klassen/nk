...
| `404 NOT_FOUND` | `findBook()`、`findNote()`、fallback middleware |
| `409 CONFLICT` | `isDuplicateEntry()` と外部キー制約エラーの変換 |
...
この場合は `409 Conflict` を返す。親を削除したときに子を連動削除する設計にはしない。削除可否は事前に `COUNT(*)` で確認せず、DB の外部キー制約で守る。

実装手順:

1. `DELETE /books/:bookId` で対象書籍が存在するか `findBook()` で確認する。
2. `DELETE FROM books WHERE id = ?` を実行する。
3. `notes` から参照されていて MySQL が外部キー制約エラーを返したら、`409 CONFLICT` に変換する。
4. 削除できたら `204 No Content` を返す。
...
1. 読書メモの詳細、更新、削除ではまず `findNote()` で対象メモを取得する。
2. `assertOwnNote()` で `notes.user_id` と `req.user.id` を比較する。
3. 一致しなければ `403 FORBIDDEN` を返す。
4. 一致した場合だけ、取得、更新、削除を続ける。
5. `GET /books/:bookId/notes` では `WHERE book_id = ? AND user_id = ?` により自分のメモだけを返す。

仕様とコードの対応:

| 仕様 | 対応するコード |
| --- | --- |
| 自分のメモだけ一覧取得 | `GET /books/:bookId/notes` の `WHERE book_id = ? AND user_id = ?` |
| 他ユーザーの読書メモは `403` | `assertOwnNote()` |
| 読書メモ作成は認証必須 | `POST /books/:bookId/notes` の `requireAuth` |
| 読書メモ更新・削除は所有者のみ | `PATCH` / `DELETE /notes/:noteId` |
